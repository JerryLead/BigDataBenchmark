package job.terasort.uncombine.compressed;
 ...
 * out-dir</b>
 */
public class BigTeraSort extends Configured implements Tool {
	private static final Log LOG = LogFactory.getLog(BigTeraSort.class);
				...
				for(int xmxIndex = 0; xmxIndex < xmxArray.length; xmxIndex++) {
					for(int xmsIndex = 0; xmsIndex < xmsArray.length; xmsIndex++) {
						for(int ismbIndex = 0; ismbIndex < ismbArray.length; ismbIndex++) {
							...
							JobConf conf = new JobConf(getConf(), BigTeraSort.class);
							...
							conf.setJobName("BigTeraSort " + splitMB + "MB " 
							...
							URI partitionUri = new URI(partitionFile.toString() + "#"
									+ TeraInputFormat.PARTITION_FILENAME);
							TeraInputFormat.setInputPaths(conf, new Path(args[0]));
							// TeraInputFormat.addInputPath(job, new Path(args[0]));
							FileOutputFormat.setOutputPath(conf, new Path(args[1]));
							
							conf.setJarByClass(BigTeraSort.class);
							conf.setOutputKeyClass(Text.class);
							conf.setOutputValueClass(Text.class);
							conf.setInputFormat(TeraInputFormat.class);
							...
							TeraOutputFormat.setFinalSync(conf, true);

							FileSystem.get(conf).delete(new Path(args[1]), true);
							FileSystem.get(conf).delete(new Path(inputDir,
	...
	public static void main(String[] args) throws Exception {						
		int res = ToolRunner.run(new BigTeraSort(), args);	
	}
