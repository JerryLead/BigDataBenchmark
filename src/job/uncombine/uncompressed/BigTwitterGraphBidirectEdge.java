...
public class BigTwitterGraphBidirectEdge {
	/**
					...
					for (int xmsIndex = 0; xmsIndex < xmsArray.length; xmsIndex++) {
						for (int ismbIndex = 0; ismbIndex < ismbArray.length; ismbIndex++) {
							...
							if (otherArgs.length != 2) {
								System.err.println("Usage: BigTwitterBiDirectEdgeCount <in> <out>");
								System.exit(2);
							}
							Job job = new Job(conf, "BigTwitterGraphBidirectEdge " + splitMB
											+ "MB " + conf.get("mapred.child.java.opts")
											+ " ismb=" + ismb + " RN=" + reduceNum);
							job.setJarByClass(BigTwitterGraphBidirectEdge.class);
