...
public class BigTwitterInDegreeCount {
	public static class TokenizerMapper extends

	Mapper<LongWritable, Text, Text, IntWritable> {

		private final static Text targetNode = new Text();
		private final static IntWritable inDegree = new IntWritable(1);
		...
		public void map(LongWritable key, Text value, Context context)
			...
			if (itr.hasMoreTokens()) {
				...
				if (itr.hasMoreTokens()) {
					...
					context.write(targetNode, inDegree);
				}
			}
						...
						for(int xmsIndex = 0; xmsIndex < xmsArray.length; xmsIndex++) {
							for(int ismbIndex = 0; ismbIndex < ismbArray.length; ismbIndex++) {
								...
								if (otherArgs.length != 2) {
									System.err.println("Usage: BigTwitterInDegreeCount <in> <out>");
									System.exit(2);
								}
								Job job = new Job(conf, "BigTwitterInDegreeCount " + splitMB + "MB " 
							    		+ conf.get("mapred.child.java.opts") 
							    		+ " ismb=" + ismb + " RN=" + reduceNum);
								
								job.setJarByClass(BigTwitterInDegreeCount.class);
